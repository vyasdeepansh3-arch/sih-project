...
// This tells our server to accept requests from any website.
app.use(cors({
    origin: '*'
}));
...
function getRecommendations(profile, allInternships) {
    ...
    return scoredInternships.filter(internship => internship.score > 0).sort((a, b) => b.score - a.score).slice(0, 5);
}
